 ...
 */
if ( !class_exists( 'Maglist_Helper' ) ) :
	class Maglist_Helper {
...
?>
		...
		?>
			<a class="skip-link screen-reader-text" href="#content">
				<?php esc_html_e( 'Skip to content', 'maglist' ); ?>
			</a>
			<?php
		}
		...
		public static function the_header_image() {
			...
			} elseif ( self::is_static_blog_page() ) {												# since 1.0
		...
		public static function get_comment_number() {
			$enable = maglist_get( 'post-comment' );
			if( !$enable ){
				return;
			}
			if ( !post_password_required() ) {
				echo '<span class="maglist-comments">';
				comments_popup_link(
		...
		public static function get_assets_version()	{
			return maglist_get( 'assets-version' );
		}
			...
			switch ( $location ) {
				...
				case 'offcanvas':
					$menu = wp_nav_menu(array(
						...
						'echo'          		 => $echo,
						'container'      	   => false,
						'menu_class'	   => 'navigation offcanvas'
					) );
					break;
				...
				case 'social-menu-header' :
					$menu = wp_nav_menu(array(
						...
						'fallback_cb' 		=> false,
						'echo' 					=> $echo,
						'menu_class'		=> 'maglist-social-menu',
						'link_before'	    => '<span>',
						'link_after'	     => '</span>',
						'depth'				   => 1
					) );
		...
		public static function posts_navigation( $echo = true )	{
			...
			if ( !$infinity_module_active || !Maglist_Helper::is_active_plugin( 'jetpack' ) ) {
				the_posts_pagination(
					array(
						'mid_size'  => 2,
						'prev_text' => esc_html__( 'Previous', 'maglist' ),
						'next_text' => esc_html__( 'Next', 'maglist' ),
					)
				);
			}
		}
		...
		public static function post_content_navigation() {
			wp_link_pages(array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'maglist' ),
				'after'       => '</div>',
		...
		public static function single_post_navigation()	{
			the_post_navigation(array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'maglist' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'maglist' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		public static function the_date( $post_id = null, $status = 'posted' ) {
            ...
            if ( $show ) :
	            ...
	            printf(
					...
					( 'posted' == $status ) ? esc_html__( 'On', 'maglist' ) : esc_html__( 'Updated on', 'maglist' ),
	                $time_tag
	            );
            endif;
        }
		...
		public static function the_breadcrumb() {
			...
			?>
				<div id="<?php echo esc_attr( self::with_prefix( 'breadcrumb' ) ) ?>" class="<?php echo esc_attr( $classes); ?>">
					<?php maglist_breadcrumb_trail( $breadcrumb_args ); ?>
				</div><!-- #breadcrumb -->
			<?php return;
			endif;
		}
				...
				<header class="entry-header">
					...
					?>
						<h2 class="entry-title">
							<?php
								printf(
									esc_html__( 'Search Results for: %s', 'maglist' ),
		...
		/**
		 * Font family used in Maglist Theme
		 *
		 * @static
		 * @access public
		 * @return object
		...
		public static function get_font_family() {
			return apply_filters( self::fn_prefix( 'standard_fonts_array' ), array(
				'font-1'  => esc_html__( 'Lato', 'maglist' ),
				'font-2'  => esc_html__( 'Oswald', 'maglist' ),
				'font-3'  => esc_html__( 'Montserrat', 'maglist' ),
				'font-4'  => esc_html__( 'Roboto', 'maglist' ),
				'font-5'  => esc_html__( 'Raleway', 'maglist' ),
				'font-6'  => esc_html__( 'Playfair Display', 'maglist' ),
				'font-7'  => esc_html__( 'Fjalla One', 'maglist' ),
				'font-8'  => esc_html__( 'Alegreya Sans', 'maglist' ),
				'font-9'  => esc_html__( 'PT Sans Narrow', 'maglist' ),
				'font-10' => esc_html__( 'Open Sans', 'maglist' ),
				'font-11' => esc_html__( 'Poppins', 'maglist' ),
			));
		}
		...
		public static function get_google_font() {
			...
			$google_fonts[] = self::uglify( self::get_font( maglist_get( 'body-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( maglist_get( 'heading-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( maglist_get( 'site-info-font' ) ) , $condition );
		...
		public static function post_format_icon(){
			...
			<?php if ( $icon ) : ?>
				<div class="maglist-post-type-icon">
