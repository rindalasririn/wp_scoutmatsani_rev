...
if ( !class_exists( 'Maglist_Customizer' ) ) {
	class Maglist_Customizer extends Maglist_Helper {
		/**
		 * The object instance.
		 *
		 * @static
		...
		public static function add_custom_control( $control ) {
			if ( !is_array( $control ) ) {
				$err = esc_html__( 'Invalid argument passed for Custom Control. Must be an array with "type" & "class" key.', 'maglist' );
				self::add_error( $err );
				return;
			}
			...
			if ( !isset( $control['type'] ) ) {
				$err = esc_html__( 'Custom Control must contain "type" key ( type of control ).', 'maglist' );
				self::add_error( $err );
				$flag = false;
			}

			if ( !isset( $control['class'] ) ) {
				$err = esc_html__( 'Custom Control must contain "class" key ( name of custom class ).', 'maglist' );
				self::add_error( $err );
				$flag = false;
			}

			if ( !isset( $control['sanitize'] ) ) {
				$err = esc_html__( 'Custom Control must contain "sanitize" key ( a callback function ).', 'maglist' );
				self::add_error( $err );
				$flag = false;
			}
		...
		public static function add_custom_section( $section ) {
			if ( !is_array( $section ) ) {
				$err = esc_html__( 'Invalid argument passed for Custom Section. Must be an array with "type" & "class" key.', 'maglist' );
				self::add_error($err);
				return;
			}
			...
			if ( !isset( $section['type'] ) ) {
				$err = esc_html__( 'Custom Section must contain "type" key ( type of section ).', 'maglist' );
				self::add_error($err);
				$flag = false;
			}

			if ( !isset( $section['class'] ) ) {
				$err = esc_html__( 'Custom Section must contain "class" key ( name of section class ).', 'maglist' );
				self::add_error( $err );
				$flag = false;
			}
				...
				foreach ( $args['fields'] as $field ) {
					...
					if ( isset( $field['id'] ) ) {
						...
						if (array_key_exists($field_id, self::$settings)) {
							...
							$msg = sprintf( esc_html__( 'Field with ID: %s already exists.', 'maglist' ), $field['id'] );
							self::add_error( $msg );
						}
		...
		public static function validate_argument( $args ) {
			...
			if ( !is_array( $args ) ) {
				...
				$msg = esc_html__( 'You must pass array as an argument in set method.', 'maglist' );
			...
			if ( !isset($args['section'] ) ) {
				...
				$msg = esc_html__( 'No section found.', 'maglist' );
				self::add_error( $msg, 2 );
			...
			if ( !is_array( $args['section'] ) ) {
				...
				$msg = sprintf(
					...
					esc_html__( 'Invalid argument', 'maglist' ),
					esc_html__( 'section', 'maglist' ),
					esc_html__( 'Must be a an array with atleast an id, title...', 'maglist' )
				);

				self::add_error( $msg, 2 );
			} elseif ( !isset( $args['section']['id'] ) ) {

				$msg = esc_html__( 'Section id is missing.', 'maglist' );
				self::add_error( $msg, 2 );
			}
			...
			if ( isset( $args['fields'] ) && !is_array($args['fields'] ) ) {
				...
				$msg = sprintf(
					'%s <strong><em>%s</em></strong>. %s',
					esc_html__( 'Invalid argument', 'maglist' ),
					esc_html__( 'fields', 'maglist' ),
					esc_html__( 'Must be a an array', 'maglist' )
				);
				...
				<div class="error">
					<?php foreach ( self::$errors as $key => $value ) : ?>
						<p>
							...
							echo sprintf(
								'<strong>%s</strong> %s',
								esc_html__( 'Maglist Customizer:', 'maglist' ),
								esc_html( $value )
							);
							?>
						</p>
...
<?php
		...
		private static function get_panel_arg( $panel ) {
			$args = array(
				'title' => empty( $panel['title'] ) ?
					esc_html__( 'No Title Specified.', 'maglist' ) :
					$panel['title'],
			);
		...
		public static function get_section_arg( $panel_id = false, $section ) {
			$args = array(
				'title' => empty($section['title']) ? esc_html__('No Title Specified.', 'maglist') : $section['title'],
			);
...
add_action( 'customize_register', array( Maglist_Customizer::get_instance(), 'register' ) );

if ( !function_exists( 'maglist_get' ) ) :
	/**
	 ...
	 * @package Maglist WordPress Theme
	 */
	function maglist_get( $id ) {
		$id = Maglist_Helper::with_prefix( $id );
		return Maglist_Customizer::get( $id );
	}
